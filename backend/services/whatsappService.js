...
<<<<<<< HEAD
  // ✅ Dynamically detect Chrome path (VERY IMPORTANT)
  getChromePath() {
    const basePath = "/opt/render/.cache/puppeteer/chrome";

    try {
      const folders = fs.readdirSync(basePath);
      if (!folders.length) throw new Error("No Chrome found");

      const latest = folders[0];
      return path.join(basePath, latest, "chrome-linux64", "chrome");
    } catch (err) {
      console.error("Chrome path detection failed:", err.message);
      return null;
    }
=======
